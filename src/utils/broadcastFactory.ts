  ...
  return {
    send: async (event: string, payload: BroadcastPayload): Promise<BroadcastResult> => {
      try {
        ...
        try {
          switch (target) {
            case 'user-specific':
              if (!sanitizedPayload.userId) {
                console.error(`❌ ${entity} broadcast missing userId for user-specific channel`);
                return { 
                  success: false, 
          ...
          console.error(`❌ Failed to generate secure channel name for ${entity}:`, channelError);
          return {
            success: false,
        ...
        try {
          ...
          if (target === 'user-specific') {
            console.error(`❌ ${entity} user-specific secure broadcast failed - NO FALLBACK for privacy protection:`, primaryError);
            return { 
              success: false, 
              error: primaryError,
              fallbackUsed: false,
          ...
          safeLog(`⚠️ Primary ${entity} secure broadcast failed, attempting secure fallback`, { primaryError }, 'warn');
          ...
          try {
            ...
            console.error(`❌ ${entity} secure fallback broadcast also failed:`, fallbackError);
        ...
        console.error(`❌ ${entity} secure broadcast failed completely:`, error);
        return { 
          success: false, 
          error,
          fallbackUsed: false
    ...
    getChannelName: (userId?: string, useFallback: boolean = false) => {
      try {
        ...
        console.error(`❌ Failed to generate secure channel name for ${entity}:`, error);
        throw error;
      }
    },
    ...
    getAuthorizedChannelNames: (userId?: string, userRole?: string) => {
      ...
      try {
        ...
        console.error(`❌ Failed to generate authorized channel names for ${entity}:`, error);
