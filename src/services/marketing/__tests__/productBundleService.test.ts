...
describe('ProductBundleService - Phase 3.2 (Mock Pattern)', () => {
  ...
  beforeEach(() => {
    ...
    InventoryService.checkProductsAvailability.mockResolvedValue({
      success: true,
      data: { available: true, stock: 100 }
    });
    InventoryService.reserveProductsForBundle.mockResolvedValue({
      success: true,
  ...
  describe('toggleBundleStatus', () => {
    it('should manage bundle activation with inventory integration', async () => {
      ...
      // Verify mock was called correctly
      expect(supabase.from).toHaveBeenCalledWith('product_bundles');
    });

    it('should validate inventory availability before activation', async () => {
      // Mock insufficient inventory
      InventoryService.checkAvailability = jest.fn().mockResolvedValue({
        success: true,
        data: {
  ...
  describe('Role-based access control', () => {
    ...
    it('should allow inventory staff read access to bundles', async () => {
      ...
      RolePermissionService.hasPermission.mockImplementation((userId, permission) => {
