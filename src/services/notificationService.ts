    ...
    switch (type) {
      case 'order_ready':
        return {
          ...
          body: `Hi ${order.customerInfo?.name || order.customer_name || 'Customer'}! Your order #${order.id.slice(-6)} is ready for pickup at ${farmStandName}. ${order.paymentMethod === 'cash_on_pickup' ? 'Please bring cash for payment. ' : ''}Pickup location: ${pickupLocation}`,
          data: {
            orderId: order.id,
            pickupLocation,
      ...
      case 'order_confirmed':
        return {
          ...
          body: `Thank you ${order.customerInfo?.name || order.customer_name || 'Customer'}! Your order #${order.id.slice(-6)} has been confirmed. We'll notify you when it's ready for pickup. ${order.pickupDate ? `Pickup date: ${order.pickupDate}` : ''}`,
          data: {
            orderId: order.id,
      ...
      case 'payment_reminder':
        return {
          ...
          body: `Hi ${order.customerInfo?.name || order.customer_name || 'Customer'}! Don't forget to bring cash ($${(order.total || order.total_amount || 0).toFixed(2)}) for your order #${order.id.slice(-6)} pickup.`,
          data: {
            orderId: order.id,
            total: order.total
          }
